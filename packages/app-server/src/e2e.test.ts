...
// Every case in this file builds a whole SessionImpl and runs a real turn
// against a tmpdir workspace: the backend and actor stacks, the voice catalogs
// looked for under a `data/voice` the tmp workspace does not have, the JSONL
// header; then the turn flushes the user block to disk, the record is
// reconciled after the provider throws, the `turn_end` meta line is appended,
// and a rewind truncates the file / the regenerate check reads it back. All
// real disk I/O; creation also fires the repository card's six git queries at
// the workspace (fire-and-forget — never awaited here). No timer, delay or
// awaited child process is on the path, so fake timers cannot make it
// deterministic; the budget is what the test assumed. The turn_end case ran
// 5.96 s under root `pnpm test` contention (~380 s on this machine) against
// vitest's 5 s default — ~0.5 s alone (1.8 s cold, as the file's first case).
// Its sibling shares the fixture, and the scripted e2e turn runs 0.9–1.3 s
// alone under what was a 15 s bound — within the same slowdown's reach — so
// all three share one bound. Bounded rather than open: a session that never
// settles must still fail, not hang the suite.
...
describe("failed turn — record re-alignment (audit 2026-07-24, H1)", () => {
  ...
  it(
    "records a turn_end marker so a failed turn is not mistaken for a crash (H1.6)",
    async () => {
      const cfg = mkConfig();
      ...
      try {
        ...
        expect(session.record.filter((b) => b.kind === "herta")).toHaveLength(
          0,
        );
      } finally {
        await session.close();
  ...
  it(
    "keeps the failed user block in the driver's record, so rewind targets IT",
    async () => {
      const cfg = mkConfig();
      const session = await mkFailingSession(cfg);
      try {
...
describe("@herta/app-server — end-to-end scripted turn", () => {
  it(
    "user message → @板砖 dispatch → approval → backend write → verdict",
    async () => {
      const cfg = mkConfig();
      ...
      const resolvedOverlays = overlayEvents.filter(
        (e) => e.kind === "resolved",
      );
